...
const DECK_ID = "telco-technical-deck";
const OUT_DIR = "C:\\Users\\PK\\Desktop\\projects\\telco_digital\\docs\\presentation\\output";
const REF_DIR = "C:\\Users\\PK\\Desktop\\projects\\telco_digital\\tmp\\slides\\telco-technical-deck\\reference-images";
...
const INK = "#EAF2FF";
const GRAPHITE = "#A9B9D3";
const MUTED = "#7890B2";
const PAPER = "#071426";
const PAPER_96 = "#10243AF5";
const WHITE = "#FFFFFF";
const ACCENT = "#22D3EE";
const ACCENT_DARK = "#67E8F9";
const GOLD = "#F5B942";
const CORAL = "#A78BFA";
...
const SOURCES = {
  architecture: "docs/LOCKED-ARCHITECTURE.md",
  features: "docs/features/README.md and capability documents 01-12",
  tests: "Repository unit, integration, and scenario tests",
};

const SLIDES = [
  { kicker: "TELCO DIGITAL · TECHNICAL WALKTHROUGH", title: "From operational events to explainable decisions", subtitle: "A shared-intelligence platform built on trustworthy facts, time-aware context, and relationship evidence.", moment: "PostgreSQL facts + Neo4j context + governed decisions", notes: "Open with the architectural thesis: intelligence is reusable across applications only when its facts, time boundaries, and evidence are trustworthy.", sources: ["architecture"] },
  { kicker: "01 · FOUNDATION", title: "One source of truth. Many intelligence consumers.", subtitle: "The stack separates authoritative facts, relationship projection, derived intelligence, and presentation.", cards: [["POSTGRESQL", "Domain facts, ledgers, immutable activity events, and the transactional outbox remain authoritative."], ["NEO4J", "An idempotent worker projects rebuildable relationship context—never changing scores as facts."], ["INTELLIGENCE", "Temporal and graph features feed twins, decisions, explanations, APIs, UI, and Copilot."]], notes: "Walk left to right. Emphasize that the graph enriches—not replaces—the relational system of record.", sources: ["architecture"] },
  { kicker: "02 · TEMPORAL CORE", title: "Correctness before intelligence", subtitle: "Every feature, prediction, and decision is bounded by the selected point in time.", metrics: [["as_of", "Hard boundary for every calculation", "No future leakage"], ["2 clocks", "occurred_at + recorded_at", "Event-time and ingestion-time"], ["immutable", "Contradictions stay in history", "Warnings surface anomalies"]], notes: "Use impossible travel as the concrete example: Singapore at 09:00 and USA at 10:00 are both stored; the warning is derived.", sources: ["architecture", "tests"] },
  { kicker: "03 · GRAPH PROJECTION", title: "Replayable graph projection", subtitle: "The transactional outbox makes relationship projection reliable, idempotent, and auditable.", cards: [["ONE TRANSACTION", "Domain write + activity event + outbox event commit or roll back together."], ["PROJECT", "The worker claims pending events and applies parameterized Cypher with idempotent MERGE operations."], ["REBUILD", "If Neo4j is corrupted, clear it and replay PostgreSQL facts and events from scratch."]], notes: "This is the operational reason the graph is safe to use: it can fail independently without corrupting authoritative state.", sources: ["architecture", "features"] },
  { kicker: "04 · WHY GRAPH", title: "Graphs reveal context hidden across joins", subtitle: "Multi-hop relationships turn isolated transactions into connected evidence.", cards: [["FRAUD", "Shared devices, shared wallets, merchant funnels, circular movement, and proximity to known fraud."], ["CUSTOMER 360", "A traversable neighbourhood across customer, device, wallet, merchant, plan, and travel context."], ["EXPLANATION", "Evidence paths show why context matters instead of returning an unexplained risk score."]], notes: "Clarify the boundary: direct facts stay relational; graph traversal is used where relationship depth is the question.", sources: ["architecture", "features"] },
  { kicker: "05 · FEATURE CONTRACT", title: "One reusable feature contract", subtitle: "CustomerFeatureService(customer_ref, as_of) joins bounded relational and graph context.", metrics: [["8 groups", "Temporal feature families", "Usage to campaigns"], ["v1", "customer-features-v1", "Versioned contract"], ["UUID5", "Deterministic snapshot identity", "Idempotent materialization"]], notes: "Explain that missing graph data is explicitly unavailable—not silently converted to zero.", sources: ["features"] },
  { kicker: "06 · CUSTOMER INTELLIGENCE", title: "Memory → behaviour → churn", subtitle: "Each layer adds interpretation while preserving the evidence below it.", cards: [["EVENT MEMORY", "Raw travel events become episodes; retrieval prioritizes the customer's own similar history."], ["BEHAVIOUR", "Evidence-backed segments summarize patterns without replacing the underlying facts."], ["CHURN", "An interpretable baseline is compared with a stronger model; predictions remain auditable outputs."]], notes: "Discuss the separation fact → feature → inference → prediction and why it improves debugging and governance.", sources: ["features"] },
  { kicker: "07 · ACTIONABLE INTELLIGENCE", title: "Three capabilities, one governance model", subtitle: "Recommendations, fraud, and forecasting propose evidence-backed actions without executing policy.", cards: [["RECOMMEND", "Generate only from the real catalogue, rank candidates, and expose uncertainty modes."], ["GRAPH FRAUD", "Combine transaction rules with shared-device, neighbourhood, and two-hop graph risk."], ["SFA FORECAST", "Benchmark naive and moving-average forecasts before translating stockout risk into proposed actions."]], notes: "Contrast the three domains while highlighting the common contract: evidence, uncertainty, and governed action.", sources: ["features"] },
  { kicker: "08 · GOVERNANCE", title: "The digital twin assembles context; the decision engine governs action", subtitle: "A computed twin provides a point-in-time view without becoming another source of truth.", metrics: [["9 lenses", "Observed to Recommended", "Unknown stays visible"], ["1 context", "Customer + selected as_of", "Computed on demand"], ["NBA", "Rules + rank + confidence", "Alternatives explained"]], notes: "List the twin lenses: Observed, Recent, Historical, Relationships, Inferred, Predicted, Unknown, Recommended, Warnings.", sources: ["architecture", "features"] },
  { kicker: "09 · DELIVERED EXPERIENCE", title: "Delivered technical surface", subtitle: "FastAPI, the showcase UI, and Copilot expose the evidence chain for discussion.", cards: [["FASTAPI", "Stable typed routes wrap application services; route files contain almost no business logic."], ["SHOWCASE UI", "Intelligence overview, customer 360, graph explorer, and capability status expose the evidence chain."], ["COPILOT", "Natural-language presentation sits over structured intelligence; it is not a source of facts or scores."]], notes: "Close by inviting discussion in order: temporal core, graph, features, memory, models, twin, decisions, Copilot.", sources: ["features", "tests"] },
];
...
function addReferenceCaption(slide, slideNo) {
  addText(
    slide,
    slideNo,
    "TELCO DIGITAL  ·  SHARED INTELLIGENCE POC",
    64,
    674,
    980,
    22,
...
async function slideCover(presentation) {
  ...
  addShape(slide, "rect", 0, 0, W, H, "#071426F2", TRANSPARENT, 0, { slideNo, role: "cover contrast overlay" });
...
async function slideCards(presentation, idx) {
  ...
  addShape(slide, "rect", 0, 0, W, H, "#071426F2", TRANSPARENT, 0, { slideNo: idx, role: "content contrast overlay" });
...
async function slideMetrics(presentation, idx) {
  ...
  const slide = presentation.slides.add();
  await addPlate(slide, idx);
  addShape(slide, "rect", 0, 0, W, H, "#071426F2", TRANSPARENT, 0, { slideNo: idx, role: "metrics contrast overlay" });
  addHeader(slide, idx, data.kicker, idx, SLIDES.length);
