...
def test_manifest_keeps_fastapi_and_simulator_not_started() -> None:
    ...
    assert by_number["07"].status == "Not started"
    assert by_number["08"].status == "Not started"
    assert by_number["09"].status == "Not started"
...
def test_manifest_does_not_claim_showcase_completes_later_capabilities() -> None:
    ...
    assert all(
        ...
        if item.number not in {"00", "01", "02", "03", "04", "05", "06", "10", "11"}
    )
